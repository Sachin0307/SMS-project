...
class resultClass:
    def __init__(self, root):
        ...
        #========title=========
        title = Label(self.root, text="Add Student Result", padx=10, compound=LEFT,
        ...
        #=========BUTTON==========
        btn_add = Button(self.root, text="Submit", font=("times new roman", 15, "bold"), bg="lightgreen", activebackground="lightgreen",
                            cursor="hand2",command=self.add).place(x=300, y=420, width=120, height=35)
        btn_clear = Button(self.root, text="Clear", font=("times new roman", 15, "bold"), bg="lightgray", activebackground="lightgray",
                            cursor="hand2").place(x=430, y=420, width=120, height=35)
    ...
    def add(self):
        ...
        try:
            if self.var_name.get() == "":
                ...
                cur.execute("select * from result where roll=? and course=?", (self.var_course.get(),self.var_course.get(),))
                row = cur.fetchone()
                if row != None:
